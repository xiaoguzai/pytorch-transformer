...
#Download pretrain-file from https://huggingface.co/google/mT5-base/tree/main
import torch
...
class T5Generation(nn.Module):
    def __init__(self,config,**kwargs):
        super(T5Generation,self).__init__()
        self.config = config
        self.t5 = T5(config)
    ...
    def forward(self,input_ids,labels=None,generate=True,layer_key_value_list=None,cross_key_value_list=None):
        ...
        if generate == False:
            ...
            if labels == None:
                ...
                decoder_ids = labels
                decoder_ids.to(labels.device)
        self.t5.to(input_ids.device)
        output_ids,layer_key_value_list,cross_key_value_list = self.T5(input_ids,labels=decoder_ids,layer_key_value_list=layer_key_value_list,\
                                                                        cross_key_value_list=cross_key_value_list)
...
class T5(nn.Module):
    def __init__(self,config,**kwargs):
        ...
        self.config = config
        self.t5encoder = T5Encoder(config)
        self.t5decoder = T5Decoder(config)
        #可以在T5模型之中加入贪婪搜索和**搜索
    ...
    def forward(self,input_ids,labels=None,layer_key_value_list=None,cross_key_value_list=None):
        ...
        encoderoutput,_ = self.t5encoder(input_ids,encoder_attention_mask)
        output_ids,layer_key_value_list,cross_key_value_list = self.t5decoder(input_ids=decoder_ids,encoder_output=encoderoutput,\
...
class T5Encoder(nn.Module):
    def __init__(self,
        ...
        self.config = config
        self.t5encoderembeddings_layer = nn.Embedding(config.vocab_size,config.embedding_size)
        self.t5embeddingdropout = nn.Dropout(config.dropout_rate)
        self.t5encoder_layer = nn.ModuleList()
        for _ in range(config.num_layers):
            t5encoder_layer_attention = T5EncoderTransformers(config)
            self.t5encoder_layer.append(t5encoder_layer_attention)
        self.final_layer_norm = T5LayerNorm(config.embedding_size,config.layer_norm_epsilon)
        self.final_dropout = nn.Dropout(config.dropout_rate)
        
    def forward(self,input_ids,encoder_attention_mask):
        output = self.t5encoderembeddings_layer(input_ids)
        output = self.t5embeddingdropout(output)
        position_bias = None
        past_value_list = None
        for layer_ndx in self.t5encoder_layer:
...
class T5Decoder(nn.Module):
    def __init__(self,
        ...
        self.t5decoderembeddings_layer = nn.Embedding(config.vocab_size,config.embedding_size)
        self.t5embeddingdropout = nn.Dropout(config.dropout_rate)
        #decoder调用embedding层进行操作,这里会放入新的输入,所以需要一个embedding网络层
        self.t5decoderlayer_transformers_list = nn.ModuleList()
        self.t5decodercross_transformers_list = nn.ModuleList()
        self.config = config
        self.final_layer_norm = T5LayerNorm(config.embedding_size,config.layer_norm_epsilon)
        self.final_dropout = nn.Dropout(config.dropout_rate)
        past_key_value_list = []
        for index in range(config.num_layers):
            t5decoderlayer_transformers = T5DecoderLayerTransformers(config)
            self.t5decoderlayer_transformers_list.append(t5decoderlayer_transformers)
            t5decodercross_transformers = T5DecoderCrossTransformers(config)
            self.t5decodercross_transformers_list.append(t5decodercross_transformers)
    ...
    def forward(self,input_ids,encoder_output,layer_key_value_list=None,cross_key_value_list=None):
        input_ids = self.t5decoderembeddings_layer(input_ids)
        input_ids = self.t5embeddingdropout(input_ids)
        ...
        for index in range(self.config.num_layers):
            current_decoder_layer_transformer = self.t5decoderlayer_transformers_list[index]
            current_decoder_cross_transformer = self.t5decodercross_transformers_list[index]
...
class T5DecoderLayerTransformers(nn.Module):
    def __init__(self,
                 config,
                 **kwargs):
        super(T5DecoderLayerTransformers,self).__init__()
        self.decoderlayerattention = T5DecoderLayerAttention(config)
        self.layer_norm0 = T5LayerNorm(config.embedding_size,eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5DecoderCrossTransformers(nn.Module):
    def __init__(self,
                 config,
                 **kwargs):
        super(T5DecoderCrossTransformers,self).__init__()
        self.decodercrossattention = T5DecoderCrossAttention(config)
        self.layer_norm0 = T5LayerNorm(config.embedding_size,eps=config.layer_norm_epsilon)
        self.layer_norm1 = T5LayerNorm(config.embedding_size,eps=config.layer_norm_epsilon)
        self.t5densegatedgeludense = T5DenseGatedGeluDense(config)
        self.dropout = nn.Dropout(config.dropout_rate)
        self.gelu_act = gelu_new
        self.config = config
    def forward(self,input_ids,encoder_output,past_key_value,position_bias):
        ...
        input_ids = self.t5densegatedgeludense(input_ids)
        input_ids = origin_input_ids+self.dropout(input_ids)
        return input_ids,past_key_value,position_bias

class T5EncoderTransformers(nn.Module):
#这里目前选用的是T5-1.1的版本，效果更好
    def __init__(self,
                 config,
                 **kwargs):
        super(T5EncoderTransformers,self).__init__()
        self.t5encoderlayerattention = T5EncoderLayerAttention(config)
        self.t5layernorm0 = T5LayerNorm(config.embedding_size,config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
        #这里注意使用nn.Dropout,F.Dropout之中有坑
        self.t5densegatedgeludense = T5DenseGatedGeluDense(config)
        self.t5layernorm1 = T5LayerNorm(config.embedding_size,config.layer_norm_epsilon)
    
    def forward(self,input_ids,position_bias,encoder_attention_mask):
        origin_input_ids = input_ids
        input_ids = self.t5layernorm0(input_ids)
        input_ids,position_bias = self.t5encoderlayerattention(input_ids,position_bias,encoder_attention_mask)
        input_ids = origin_input_ids+self.dropout(input_ids)
        #到这里也是一样的
        origin_input_ids = input_ids
        input_ids = self.t5layernorm1(input_ids)
        input_ids = self.t5densegatedgeludense(input_ids)
        input_ids = origin_input_ids+self.dropout(input_ids)
        #还是这里的self.dropout出问题!!!
        return input_ids,position_bias

class T5EncoderLayerAttention(nn.Module):
    def __init__(self,
                 config,
                 **kwargs):
        super(T5EncoderLayerAttention,self).__init__()
        self.query_layer = nn.Linear(config.embedding_size,config.num_heads*config.size_per_head,bias=False)
        #这里T5-base的时候config.num_heads*config.size_per_head == config.embedding_size,
        #T5-small的时候两者并不相等
...
class T5DecoderLayerAttention(nn.Module):
    def __init__(self,
                 config,
                 **kwargs):
        super(T5DecoderLayerAttention,self).__init__()
        self.query_layer = nn.Linear(config.embedding_size,config.num_heads*config.size_per_head,bias=False)
        #这里T5-base的时候config.num_heads*config.size_per_head == config.embedding_size,
        #T5-small的时候两者并不相等
        self.key_layer = nn.Linear(config.embedding_size,config.num_heads*config.size_per_head,bias=False)
...
class T5DecoderCrossAttention(nn.Module):
    def __init__(self,
                 config,
                 **kwargs):
        super(T5DecoderCrossAttention,self).__init__()
        self.query_layer = nn.Linear(config.embedding_size,config.num_heads*config.size_per_head,bias=False)
        #这里T5-base的时候config.num_heads*config.size_per_head == config.embedding_size,
        #T5-small的时候两者并不相等
        self.key_layer = nn.Linear(config.embedding_size,config.num_heads*config.size_per_head,bias=False)
