...
class AttentionLayer(nn.Module):
    def __init__(self,
        ...
        self.maxlen = maxlen
        self.max_relative_position = max_relative_position
        self.relative_positions_encoding = self.relative_position_encoding(length=self.maxlen,
    ...
    def forward(self,inputs,mask=None,**kwargs):
        ...
        #query = (12,1,5,64)
        query = query.contiguous().view([seq_len,batch_size*self.num_attention_heads,self.size_per_head])
        #query = (12,5,64)
        ...
        current_results = current_results.view([batch_size,self.num_attention_heads,seq_len,seq_len])
        ...
        attention_scores = attention_scores.contiguous().view(seq_len,batch_size*self.num_attention_heads,seq_len)
        ...
        attention_scores = attention_scores.view(batch_size,self.num_attention_heads,seq_len,self.size_per_head)
        context_layer = attention_scores+context_layer
        context_layer = context_layer.permute(0,2,1,3)
        context_layer = context_layer.view(batch_size,seq_len,self.num_attention_heads*self.size_per_head)
